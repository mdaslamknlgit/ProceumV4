...
//check "frmViewContent_Load" for main code
namespace Proceum.Forms
{
    internal partial class frmViewContent : MetroForm
    {
        public string PURL = "";
        ...
        public frmViewContent(string URL)
        {
            ...
            try
            {
                ...
                //MessageBox.Show("Firefox Directory Location " + FireFoxDirectoryFullPath);
                ...
                //Xpcom.Initialize(FireFoxDirectoryFullPath);

                //MessageBox.Show("Firefox Initialized" + FireFoxDirectoryFullPath);
        ...
        long Report_View_Time = 0;

        string Report_Course_Id = "";
        string Report_Video_Id = "";
        string Report_Is_VOD = "";
        string Report_Remaining_Time = "0";




        double? SavedTime = null;
        ...
        private void bwLoadContent_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                // url = PlayEncryptedVideo(FileName);
            }
            catch (Exception ex)
            {
                //_logger.Error(ex);
        ...
        private void bwLoadContent_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                resetContentArea();
                MessageBox.Show("Offline file not found. Please download it again.");
                return;
            }
        ...
        private void BackgroundPDFDownloader_DoWork(object sender, DoWorkEventArgs e)
        {
            //try
            //{
            //    var pdf = e.Argument as Modals.PDFInfo;
            //    byte[] response = PDFDownloader.DownloadData(pdf.URL);

            //    Invoke(new Action(() =>
            //    {
            //        if (treeView1.Enabled == false)
            //        {
            //            pnPdf.Dock = DockStyle.Fill;
            //            pnPdf.Show();
            //            pdfViewer1.Document = OpenPDF(response);
            ...
            //        treeView1.Enabled = true;
            //        pnLoading.Visible = false;
            //        MessageBox.Show("Error in loading content.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
            //    }));
            //}
        }
        ...
        private void treeView1_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            {
                MessageBox.Show(MessageToShow, "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                MessageToShow = null;
            }
        }
        ...
        //private void geckoWebBrowser1_DocumentCompleted(object sender, Gecko.Events.GeckoDocumentCompletedEventArgs e)//this is not used in this project *important
        //{

        //    string result = Path.GetTempPath();
        //    //Log_File = new StreamWriter(result + "\\" + "Text_File.Text");

        //    if (geckoWebBrowser1.Url.AbsolutePath != "blank")
        //    {
        //        try
        //        {
        //            //pnChatting.Visible = true;
        //            //pnChatting.Dock = DockStyle.Right;
        //            watermark.Start();
        //        }
        //        catch (Exception ex)
        //        {
        ...
        //        //ResizeVideoInsidePlayer(false);

        //        //try
        //        //{
        //        //    var Video = (Modals.StreamList)SelectedNode.Tag;
        //        //    VideoValidityTimer.Enabled = true;

        //        //    if (Video.is_live != "1")
        //        //    {
        //        //        GetAllPreviousBookmarksOnVideoStart(Video.bookmark);
        //        //    }

        //        //    string Course_ID = ((Modals.CourseList)treeView1.Tag).id,
        //        //           Video_ID = Video.id,
        //        //           Is_VOD = Video.is_vod;
        //        //    Start_Generating_Report(Course_ID, Video_ID, Is_VOD);
        //        //}
        //        //catch { }
        ...
        private void frmViewContent_FormClosing(object sender, FormClosingEventArgs e)
        {
            webView21.Dispose();
        ...
        private void frmViewContent_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                //StreamWriter newfile = new StreamWriter("Logfile.text");
                //newfile.Write("File Has created");
                //_logger.Info("Log has created");


                //Test
                //proceuma:3_0_9933166671773164:8145
                ...
                WholeCoursePositiveResponseFromServer1 response = Api_Manager.GetVideolistWUserNToken(UserID,TokenStr).Result;//userid=2 static

                if (response.data != null)
                {
                    foreach (var VideoInfos in response.data)
                    {
                    ...
                    var a = response.data.FirstOrDefault();
                ...
                //Make Full Screen
                //PlayerFullScreenClick(null, null);

                //FileInfo file = new FileInfo(Offline_Video_File_Path);
                ...
                try
                {
                    ...
                    //foreach (Control ctrl in metroTabControl1.Controls)
                    //{
                    //    if (ctrl is Button)
                    //    {
                    //        btn = ctrl as Button;
        ...
        //private void PDFExtractor_DocumentCompleted(object sender, Gecko.Events.GeckoDocumentCompletedEventArgs e)
        //{

        //}

        private void frmViewContent_SizeChanged(object sender, EventArgs e)
        {
           // metroTabControl1.Size = new Size(this.Width, metroTabControl1.Height);
        }
        ...
        private void treeView1_DrawNode(object sender, DrawTreeNodeEventArgs e)
        {
            ...
            if (e.Node.IsVisible)
            {
                string[] Splited = e.Node.Text.Split('\n');
                if (Splited.Length > 0)
                {
                    Brush foreColour;
                    Brush backColour;

                    Rectangle rect = new Rectangle(e.Bounds.X, e.Bounds.Y, Convert.ToInt32(CreateGraphics().MeasureString(e.Node.Text, treeView1.Font).Width) + 5, treeView1.ItemHeight);

                    if (e.Node.IsSelected)
                    {
                        foreColour = Brushes.White;

                        backColour = new SolidBrush(Color.FromArgb(21, 118, 140));
                    }
                    else
                    {
                        foreColour = Brushes.Black;
                        backColour = Brushes.White;
                    }
                    ...
                    try
                    {
                        ...
                        if (Splited.Length > 1) format = new StringFormat { LineAlignment = StringAlignment.Near, Alignment = StringAlignment.Near };
                        else format = new StringFormat { LineAlignment = StringAlignment.Center, Alignment = StringAlignment.Near };
                        e.Graphics.DrawString(Splited[0], treeView1.Font, foreColour, rect, format);

                        if (Splited.Length > 1)
                        {
                            Font SmallFont = new Font(treeView1.Font.FontFamily, treeView1.Font.Size - 4);

                            format = new StringFormat { LineAlignment = StringAlignment.Far, Alignment = StringAlignment.Near };
                            e.Graphics.DrawString(Splited[1], SmallFont, foreColour, rect, format);
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private void TimerValidityUpdate_Tick(object sender, EventArgs e)//important
        {
            UpdateVideoValidityFunction();
        }
        ...
        {
            MainSettingMenuStrip.Renderer = new MyRenderer();
            Video_Menu_Strip.Renderer = new MyRenderer();
        }
        ...
        private void pnLoading_VisibleChanged(object sender, EventArgs e)
        {
            pnLoading.Location = new Point((pnLoading.Parent.Width / 2) - (pnLoading.Width / 2), (pnLoading.Parent.Height / 2) - (pnLoading.Height / 2));
        }

        private void splitContainer1_Panel2_Resize(object sender, EventArgs e)
        {
            // pnPlayer.Width = splitContainer1.Panel2.Width - (pnChatting.Visible ? pnChatting.Width + 7 : 0);
        }

        private void pnPlayer_Resize(object sender, EventArgs e)
        {
            //ResizeVideoInsidePlayer(true);
        }
        ...
        private void Refrence_Panel_Resize(object sender, EventArgs e)
        {

        }
        private void btn_DownloadVideo_Click(object sender, EventArgs e)
        {
        ...
        private void Video_Menu_Strip_Opening(object sender, CancelEventArgs e)
        {
